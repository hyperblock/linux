...
static struct loop_func_table xor_funcs = {
    .number = LO_CRYPT_XOR, .transfer = transfer_xor, .init = xor_init};
...
static int lo_write_transfer(struct loop_device *lo, struct request *rq,
  ...
  rq_for_each_segment(bvec, rq, iter) {
    ret = lo_do_transfer(lo, WRITE, page, 0, bvec.bv_page, bvec.bv_offset,
                         bvec.bv_len, pos >> 9);
    if (unlikely(ret))
      break;
...
static int lo_read_simple_mfile_try(struct loop_device *lo, struct request *rq,
  ...
  rq_for_each_segment(bvec, rq, iter) {
    ...
    pr_info("Before lsmt_iter_pread, pos is %lx, bvec.bv_len is %lx ", pos,
            bvec.bv_len);
...
static int lo_read_transfer(struct loop_device *lo, struct request *rq,
  ...
  rq_for_each_segment(bvec, rq, iter) {
    ...
    ret = lo_do_transfer(lo, READ, page, 0, bvec.bv_page, bvec.bv_offset, len,
                         offset >> 9);
    if (ret)
      goto out_free_page;
...
static int lo_rw_aio_mfile(struct loop_device *lo, struct loop_cmd *cmd,
  ...
  // Oirginally iter will do all bvecs in a rq, but here we'll do one bvec at a
  // time.
...
static inline void loop_update_dio_mfile(struct loop_device *lo) {
  __loop_update_dio_mfile(lo,
                          io_is_direct(lo->lo_backing_files[0]) | lo->use_dio);
}
...
static void loop_reread_partitions(struct loop_device *lo,
  ...
  if (rc)
    pr_warn("%s: partition scan of loop%d (%s) failed (rc=%d)\n", __func__,
            lo->lo_number, lo->lo_file_name, rc);
}
...
static ssize_t loop_attr_show(struct device *dev, char *page,
                              ssize_t (*callback)(struct loop_device *,
                                                  char *)) {
...
static int loop_prepare_queue(struct loop_device *lo) {
  ...
  lo->worker_task =
      kthread_run(loop_kthread_worker_fn, &lo->worker, "loop%d", lo->lo_number);
...
static int loop_set_fd(struct loop_device *lo, fmode_t mode,
  ...
  set_blocksize(bdev,
                S_ISBLK(inode->i_mode) ? block_size(inode->i_bdev) : PAGE_SIZE);
...
* note that we will fput rather than filp_close when we call loop_clr_fd_mfile
...
loop_init_lsmtfile(struct loop_device *lo,
  ...
  mfds = kvmalloc(sizeof(struct loop_mfile_fds) + sizeof(int) * IMAGE_RO_LAYERS,
                  GFP_KERNEL);
  if (!access_ok(VERIFY_READ, arg->fds, sizeof(int) * IMAGE_RO_LAYERS))
    return NULL;
  // now copy all the mem from userspace
  if (copy_from_user(mfds, arg, sizeof(struct loop_mfile_fds) +
...
static void loop_alloc_mfile(struct loop_mfile *mf, size_t fcnt) {
  ...
  for (i = 0; i < fcnt; i++) {
    mf->filenames[i] =
        (char *)kvzalloc(LO_NAME_SIZE * sizeof(char), GFP_KERNEL);
  }
}
...
static void loop_copy_mfile(const struct loop_mfile *src,
  ...
  for (i = 0; i < n; i++) {
    ...
    // memcpy((char *)src->filenames[i], (char *)dst->filenames[i],
    // LO_NAME_SIZE);
...
static int loop_set_fd_mfile(struct loop_device *lo, fmode_t mode,
  ...
  // lo->lo_backing_file = file;
  lo->lo_backing_files =
      (struct file **)lsmtfile->m_files; // for operating in loop_clr_fd_mfile
  lo->transfer = NULL;
  lo->ioctl = NULL;
  lo->lo_sizelimit = 0;
  lo->old_gfp_mask = mapping_gfp_mask(mapping);
...
static int loop_set_status(struct loop_device *lo,
  ...
  if (info->lo_encrypt_key_size) {
    memcpy(lo->lo_encrypt_key, info->lo_encrypt_key, info->lo_encrypt_key_size);
    lo->lo_key_owner = uid;
  }
...
static int loop_get_status(struct loop_device *lo, struct loop_info64 *info) {
  ...
  if (lo->lo_encrypt_key_size && capable(CAP_SYS_ADMIN)) {
    ...
    memcpy(info->lo_encrypt_key, lo->lo_encrypt_key, lo->lo_encrypt_key_size);
  }

  /* Drop lo_ctl_mutex while we call into the filesystem. */
  file = get_file(lo->lo_backing_file);
  mutex_unlock(&lo->lo_ctl_mutex);
  ret = vfs_getattr(&file->f_path, &stat, STATX_INO, AT_STATX_SYNC_AS_STAT);
  if (!ret) {
...
static int loop_set_status64_mfile(struct loop_device *lo,
  ...
  for (i = 0; i < n; i++) {
    // if(!access_ok(VERIFY_READ, arg->mfile.filenames[i], LO_NAME_SIZE)) return
    // NULL;
    if (!access_ok(VERIFY_READ, (char *)info64.mfile.filenames[i],
                   LO_NAME_SIZE))
      return NULL;
    // if(copy_from_user((char *)fns[i], arg->mfile.filenames[i], LO_NAME_SIZE))
    // return NULL;
    if (copy_from_user((char *)fns[i], (char *)info64.mfile.filenames[i],
                       LO_NAME_SIZE))
      return NULL;
  }
...
static int loop_get_status64_mfile(struct loop_device *lo,
  ...
  n = info64.mfile.mfcnt;
  for (i = 0; i < n; i++) {
    if (copy_to_user(arg->mfile.filenames[i], info64.mfile.filenames[i],
                     LO_NAME_SIZE))
      return -EFAULT;
  }
...
static int lo_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd,
  ...
  case LOOP_SET_FD_MFILE:
    pr_info("here am i \n");
    err =
        loop_set_fd_mfile(lo, mode, bdev, (struct loop_mfile_fds __user *)arg);
    break;
  case LOOP_CHANGE_FD:
  ...
  case LOOP_SET_STATUS64_MFILE:
    err = -EPERM;
    if ((mode & FMODE_WRITE) || capable(CAP_SYS_ADMIN))
      err = loop_set_status64_mfile(lo, (struct loop_info64 __user *)arg);
    break;
...
static int lo_compat_ioctl(struct block_device *bdev, fmode_t mode,
  ...
  case LOOP_GET_STATUS:
    err = mutex_lock_killable(&lo->lo_ctl_mutex);
    if (!err) {
      err = loop_get_status_compat(lo, (struct compat_loop_info __user *)arg);
      /* loop_get_status() unlocks lo_ctl_mutex */
    }
    break;
  case LOOP_SET_CAPACITY:
...
static blk_status_t loop_queue_rq(struct blk_mq_hw_ctx *hctx,
  ...
  pr_info(
      "==================================in loop_queue_rq request is %lx\n ",
      rq);
  struct loop_cmd *cmd = blk_mq_rq_to_pdu(rq);
...
static struct miscdevice loop_misc = {
    .minor = LOOP_CTRL_MINOR, .name = "loop-control", .fops = &loop_ctl_fops,
};
